...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    pylab.title('Virus Population over Timesteps')
    pylab.legend()
    pylab.show()
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        pop_densisty = len(self.viruses) / float(self.maxPop)

        for virus in self.viruses:
            try:
                new_virus = virus.reproduce(pop_densisty, self.drugs)
                reproduced_viruses.append(new_virus)
            except NoChildException:
                continue
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    meanPopOfViruses = [0] * (300 + 150)
    meanPopOfResistViruses = [0] * 300
    
    for trial in range(numTrials):
        viruses = [ResistantVirus(maxBirthProb, clearProb, resistances, mutProb)] * numViruses
        patient = TreatedPatient(viruses, maxPop)
        for i in range(150):
            meanPopOfViruses[i] += patient.update()
            meanPopOfResistViruses[i] += patient.getResistPop(['guttagonol'])
        patient.addPrescription('guttagonol')
        for i in range(150, 300):
            meanPopOfViruses[i] += patient.update()
            meanPopOfResistViruses[i] += patient.getResistPop(['guttagonol'])
    
    meanPopOfViruses = [item / float(numTrials) for item in meanPopOfViruses]
    meanPopOfResistViruses = [item / float(numTrials) for item in meanPopOfResistViruses]
    
    pylab.plot(meanPopOfViruses, label="Resistant virus")
    pylab.plot(meanPopOfResistViruses, label="Guttagonol-resistant virus")
...
def simulationDelayedTreatment(numTrials):
    ...
    for trial in range(numTrials):
        ...
        for i in range(delay):
            totalPopViruses[trial] = patient.update()
    ...
    pylab.hist(totalPopViruses)
    pylab.title('Delay')
    pylab.xlabel('# of Virus Population')
